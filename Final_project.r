...
datapath <- "/work/gene8940/fg69001/Final_project/kallisto" # NOTE: you need to modify this line to match the path made by your BASH script
resultdir <- "/work/gene8940/fg69001/Final_project" # NOTE: you need to modify this line to match the path made by your BASH script
setwd(resultdir)

#create a sample-to-condition metadata object
sample <- c("CCI1", "CCI21", "CCI3", "CNI1", "CNI2", "CNI11") # create vector of sample IDs
kallisto_dirs <- file.path(datapath, sample) # create vector of paths to kallisto output directories
condition <- c("Infected", "Infected", "Infected", "Non-infected", "Non-infected", "Non-infected") # create vector of treatments in same order as sample IDs
...
write.csv(x = sleuth_significant, file = "infected_vs_noninfected_sleuth_q_0.05.csv", row.names = FALSE)
